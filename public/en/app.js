  ...
  for (const item of displayCases) {
    ...
    card.innerHTML = `
      ...
      <div class="case-details">
        ...
        <div class="detail-row"><span class="detail-label">Court:</span> <span class="detail-value">${item.court_name || '---'}</span></div>
        <div class="detail-row"><span class="detail-label">Prosecutor:</span> <span class="detail-value">${item.prosecutor || '---'}</span></div>
        <div class="detail-row"><span class="detail-label">Board:</span> <span class="detail-value">${item.judge || '---'}</span></div>
        <div class="detail-row"><span class="detail-label">Date:</span> <span class="detail-value">${item.date || '---'}</span></div>
        <div class="detail-row"><span class="detail-label">Defendant:</span> <span class="detail-value">${item.defendantCount || '0'}</span></div>
      </div>
      <div class="card-footer">
        <div class="btn-map">View Map</div>
      </div>
    `;
    card.addEventListener("click", () => {
      window.location.href = `/en/map.html?caseId=${item.id}`;
    });
...
async function buildPreview(caseId) {
  ...
  if (!previewNetwork) {
    ...
    previewNetwork.on("click", (params) => {
      if (selectedCaseId) {
        window.location.href = `/en/map.html?caseId=${selectedCaseId}`;
      }
    });
  } else {
