...
|----------|-----------|-----------|
| Forms with multiple inputs | `KeyboardAwareScrollViewCompat` | `bottomOffset`, `keyboardShouldPersistTaps="handled"` |
| FlatList with inputs in items | `FlatList` + `renderScrollComponent` | `renderScrollComponent={(props) => <KeyboardAwareScrollView {...props} />}` |
| Chat/messaging apps | `KeyboardAvoidingView` + `FlatList` | `behavior="padding"` (on KAV), `keyboardShouldPersistTaps="handled"` (on FlatList) |
| Single input (search, comment) | `KeyboardAvoidingView` | `behavior="padding"` |
...
|------|---------|-------------|
| `bottomOffset` | `0` | Extra space below focused input |
| `keyboardShouldPersistTaps` | `"handled"` | Allow tapping buttons while keyboard open |
| `enabled` | `true` | Enable/disable keyboard avoidance |
...
| Header Type | keyboardVerticalOffset | Why |
|-------------|----------------------|-----|
| No header (`headerShown: false`) | `0` | Content starts from screen top |
| Transparent header (default in Expo Router) | `0` | Content renders behind header |
| Opaque header | `headerHeight` | Content starts below header |
...
|----------|----------|-------|
| iOS | `behavior="padding"` | Smooth keyboard animation |
| Android | `behavior="padding"` | Works well with keyboard-controller |
